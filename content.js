...
function initDeepseekExporter() {
  ...
  // Preload html2canvas
  loadHtml2Canvas().catch(err => console.error('Error preloading html2canvas:', err));
...
async function exportConversation() {
  try {
    ...
    if (typeof html2canvas === 'undefined') {
      ...
      // Double check that html2canvas is now defined
      if (typeof html2canvas === 'undefined') {
...
// Function to load html2canvas dynamically
function loadHtml2Canvas() {
  return new Promise((resolve, reject) => {
    ...
    // First try to load the local copy
    const script = document.createElement('script');
    script.src = chrome.runtime.getURL('html2canvas.min.js');
    
    script.onload = () => {
      console.log('Successfully loaded local html2canvas');
      resolve();
    };
    
    script.onerror = (err) => {
      console.error('Failed to load local html2canvas, trying CDN fallbacks:', err);
      
      // Try loading from multiple CDNs for redundancy
      const cdnUrls = [
      ...
      function tryNextCdn(index) {
        if (index >= cdnUrls.length) {
          reject(new Error(`Failed to load html2canvas from all sources: ${errors.join(', ')}`));
          return;
        }
        
        const cdnScript = document.createElement('script');
        cdnScript.src = cdnUrls[index];
        
        cdnScript.onload = () => {
          console.log(`Successfully loaded html2canvas from ${cdnUrls[index]}`);
          resolve();
        };
        
        cdnScript.onerror = (cdnErr) => {
          console.error(`Failed to load html2canvas from ${cdnUrls[index]}:`, cdnErr);
          errors.push(`${cdnUrls[index]}: ${cdnErr.message || 'Unknown error'}`);
